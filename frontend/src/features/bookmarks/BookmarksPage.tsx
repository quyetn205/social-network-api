    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 space-y-3">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          {post.author && (
            <Link to={`/profile/${post.author.id}`} className="font-semibold text-gray-900 dark:text-gray-100 hover:text-blue-500">
...
export default function BookmarksPage() {
  ...
  return (
    <div className="space-y-4 max-w-xl mx-auto">
      {/* Header */}
      <h1 className="text-xl font-bold text-gray-800 dark:text-gray-100">Bài viết đã lưu</h1>
      ...
      {!isLoading && posts.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Nhấn 💾 trên bài viết để lưu lại</p>
          <Link to="/feed" className="inline-block mt-4 text-sm text-blue-500 hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300">
            ← Quay lại bảng tin
          </Link>
        </div>
      )}
