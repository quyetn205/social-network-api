...
export default function ExplorePage() {
  ...
  const { isLoading, isFetching } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const data = await postsApi.explore(topicRef.current ?? undefined, cursorRef.current ?? undefined, LIMIT)
  ...
  return (
    <div className="space-y-4 max-w-xl mx-auto">
      <h1 className="text-xl font-bold text-gray-900 dark:text-dark-text">Khám phá</h1>
      ...
      {allPosts.length > 0 && (
        <div ref={loadMoreRef} className="py-4 text-center">
          ...
          {!hasMoreRef.current && (
            <p className="text-gray-400 dark:text-dark-muted text-sm">Hết bài viết</p>
          )}
        </div>
      )}
    </div>
