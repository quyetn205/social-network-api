...
export default function ProfilePage() {
  ...
  // Sync follow state from query
  useEffect(() => {
    if (!isOwnProfile && followStatus !== undefined) {
  ...
  const followMutation = useMutation({
    mutationFn: () => following
      ? usersApi.unfollowUser(uid)
      : usersApi.followUser(uid),
    onMutate: () => {
      setFollowing(!following)
    },
    onError: () => {
      setFollowing(followStatus?.following ?? false)
    },
    onSettled: () => {
      ...
      <div className="bg-white dark:bg-dark-card rounded-xl border border-gray-200 dark:border-dark-border p-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            {user.date_of_birth && (
              <p className="text-gray-400 dark:text-dark-muted text-xs mt-1">Sinh nhật: {user.date_of_birth}</p>
            )}
          </div>
          ...
          {!isOwnProfile && (
            ...
            >
              {followMutation.isPending ? '...' : following ? 'Đã theo dõi' : 'Theo dõi'}
            </button>
          )}
        </div>

        <div className="flex gap-6 mt-4 pt-4 border-t border-gray-100 dark:border-dark-border">
          <button className="text-center cursor-pointer" onClick={() => setTab('posts')}>
            <div className={`text-lg font-bold ${tab === 'posts' ? 'text-blue-500' : 'text-gray-900 dark:text-dark-text'}`}>{user.posts_count ?? '-'}</div>
            <div className="text-xs text-gray-400 dark:text-dark-muted">Bài viết</div>
          </button>
          <button className="text-center cursor-pointer" onClick={() => setTab('followers')}>
            <div className={`text-lg font-bold ${tab === 'followers' ? 'text-blue-500' : 'text-gray-900 dark:text-dark-text'}`}>{user.followers_count ?? '-'}</div>
            <div className="text-xs text-gray-400 dark:text-dark-muted">Người theo dõi</div>
          </button>
          <button className="text-center cursor-pointer" onClick={() => setTab('following')}>
            <div className={`text-lg font-bold ${tab === 'following' ? 'text-blue-500' : 'text-gray-900 dark:text-dark-text'}`}>{user.following_count ?? '-'}</div>
            <div className="text-xs text-gray-400 dark:text-dark-muted">Đang theo dõi</div>
          </button>
        </div>
      </div>
        ...
        <div className="p-4">
          {tab === 'posts' && (
            <div className="space-y-3">
              {loadingPosts && [...Array(2)].map((_, i) => <PostCardSkeleton key={i} />)}
              {!loadingPosts && (!userPosts?.items || userPosts.items.length === 0) && (
          ...
          {tab === 'followers' && (
            <div className="space-y-3">
              ...
              {!loadingFollowers && (!followersData?.items || followersData.items.length === 0) && (
                <div className="text-center py-8 text-gray-400 dark:text-dark-muted">
          ...
          {tab === 'following' && (
            <div className="space-y-3">
              ...
              {!loadingFollowing && (!followingData?.items || followingData.items.length === 0) && (
                <div className="text-center py-8 text-gray-400 dark:text-dark-muted">
