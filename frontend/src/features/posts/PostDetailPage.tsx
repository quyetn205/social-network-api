...
export default function PostDetailPage() {
  ...
  const bookmarkMutation = useMutation({
    mutationFn: () => bookmarked ? bookmarksApi.unbookmark(Number(postId)) : bookmarksApi.bookmark(Number(postId)),
    onMutate: () => {
      setBookmarked(!bookmarked)
      showToast(bookmarked ? 'Đã bỏ lưu bài viết' : 'Đã lưu bài viết', 'success')
    },
    onError: () => {
      setBookmarked(bookmarked)
  ...
  return (
    <div className="max-w-xl mx-auto space-y-4">
      {/* Back */}
      <Link to="/feed" className="text-sm text-blue-500 hover:underline dark:text-blue-400">
        ← Quay lại bảng tin
      </Link>
      ...
      <div className="bg-white dark:bg-dark-card rounded-xl border border-gray-200 dark:border-dark-border p-5 space-y-3">
        ...
        <p className="text-gray-800 dark:text-dark-text whitespace-pre-wrap">{post.content}</p>
      ...
      <div className="space-y-4">
        ...
        {topLevel.map((comment: Comment) => (
          <div key={comment.id} className="bg-white dark:bg-dark-card rounded-xl border border-gray-200 dark:border-dark-border p-4 space-y-3">
